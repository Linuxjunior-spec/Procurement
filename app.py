...
def upload_to_google_drive(local_file_path, folder_id="1nFRmM0bHc4Hn5B3fXTwK1rT-0Rb-0RIC"):
    try:
        ...
        # ค้นหาเพื่อลบไฟล์เวอร์ชันเก่าใน Drive ออกก่อน
...
def download_from_google_drive(file_name, local_file_path, folder_id="1nFRmM0bHc4Hn5B3fXTwK1rT-0Rb-0RIC"):
    try:
        drive = get_google_drive_service()
        if not drive: return False
        ...
        file_list = drive.ListFile({'q': f"'{folder_id}' in parents and title = '{file_name}' and trashed = false"}).GetList()
        if file_list:
            # ดึงไฟล์ตัวล่าสุดบน Drive ลงมาเขียนทับไฟล์ชั่วคราวบนคลาวด์
        ...
        if not st.session_state.pur_proposals:
            ...
            for proposal in st.session_state.pur_proposals:
                ...
                summary_pur_data.append({
                    ...
                    "ผู้ร้องขอโครงการ": proposal.get("requestor", "-"),
            ...
            if selected_pur_option:
                ...
                if not st.session_state.item_codes_master:
                    ...
                    if st.button("➕ กดเพื่อเพิ่มลำดับวัสดุรายการนี้เข้า BOQ", use_container_width=True, type="primary"):
                        ...
                        curr_pur_obj["items"].append({
                            ...
                            "brand": input_brand.strip() if input_brand else "-",
                            "remark": input_remark.strip() if input_remark else ""
                        })
                ...
                if curr_pur_obj.get("items"):
                    ...
                    try:
                        ...
                        cat_style = ParagraphStyle('CatStyle', fontName='DBHeavent', fontSize=13, leading=15, alignment=0, textColor=colors.HexColor("#003311"))
                    ...
                    # ส่วนที่ 3: ตารางรายการพัสดุสินค้าจัดซื้อ (7 คอลัมน์สมบูรณ์แบบ)
                    table_data = [[
                    ...
                    for cat in categories_in_doc:
                        ...
                        table_styles_list.append(('BACKGROUND', (0, current_row_idx), (-1, current_row_idx), colors.HexColor("#C2EABD")))
                    ...
                    # ส่วนที่ 5: บล็อกลงนามลายเซ็นท้ายเล่ม
                    signature_data = [
                        ["(......................................................)", "", "(......................................................)"],
            ...
            if st.form_submit_button("💾 เปิดเล่ม / บันทึกตั้งต้นเอกสารโปรเจกต์นี้"):
                if new_pur_id and new_pur_project and new_pur_client and selected_pur_requestor:
                    st.session_state.pur_proposals.append({
                        ...
                        "requestor": selected_pur_requestor.strip(),
